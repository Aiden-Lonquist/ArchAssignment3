...
@interface Renderer () {
    ...
    GLuint brickVertexArray, ballVertexArray, playerVertexArray;   // vertex arrays for brick and ball
    int numBrickVerts, numBallVerts, numPlayerVerts;
...
- (void)update
{
    ...
    b2Vec2 *thePlayer = (((*objPosList).find("player") == (*objPosList).end()) ? nullptr : &(*objPosList)["player"]);
    ...
    {
        // Set up VAO/VBO for brick
        glGenVertexArrays(1, &playerVertexArray);
        ...
        int k = 0;
        numPlayerVerts = 0;
        vertPos[k++] = thePlayer->x - Player_Width/2;
        vertPos[k++] = thePlayer->y + Player_Height/2;
        vertPos[k++] = 10;  // z-value is always set to same value since 2D
        numPlayerVerts++;
        vertPos[k++] = thePlayer->x + Player_Width/2;
        vertPos[k++] = thePlayer->y + Player_Height/2;
...
- (void)draw:(CGRect)drawRect;
{
    ...
    if (thePlayer)
        printf("Player: (%5.3f,%5.3f)", thePlayer->x, thePlayer->y);
    printf("\n");
#endif
